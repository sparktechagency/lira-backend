...
const getVotedPosts = async (userId: string, query: Record<string, unknown>) => {
     ...
     if (searchTerm) {
          searchQuery.$or = [{ title: { $regex: searchTerm, $options: 'i' } }, { description: { $regex: searchTerm, $options: 'i' } }];
     }
     ...
     const posts = await CommunityModel.find(searchQuery).populate('userId', 'name email').sort(sortOptions).skip(skip).limit(limit).lean();

     // Create a map of user votes for quick lookup
     const voteMap = new Map(userVotes.map((vote) => [vote.postId.toString(), vote.vote]));
...
const getMyPosts = async (userId: string, query: Record<string, unknown>) => {
     const queryBuilder = new QueryBuilder(CommunityModel.find({ userId }), query).filter().sort().paginate().fields().search(['title', 'description']);
